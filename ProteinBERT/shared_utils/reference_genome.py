...
class GenomeReader:
    ...
    A GenomeReader object is initialized from a directory containing the relevant FAST files.
    It will automatically detect all .fa and .fasta files within that directory.
    The reference genome sequences of all human chromosomes (chrXXX.fa.gz files) can be downloaded from UCSC's FTP
    site at:
    ...
    IMPORTANT: In version hg19 there's an inconsistency in the reference genome of the M chromosome between
    UCSC and RegSeq/GENCODE, so the file chrM.fa is better to be taken from RefSeq (NC_012920.1) instead of UCSC, from:
    ...
    def __init__(self, ref_genome_dir):
        ...
        self.chromosome_readers = dict(
            self._create_chromosome_reader(file_name) for file_name in os.listdir(self.ref_genome_dir)
...
def _is_ref_genome_file(file_name):
    ...
    for _ in _SUPPORTED_REF_GENOME_EXTENSIONS:
