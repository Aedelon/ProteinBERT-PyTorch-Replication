...
* Link: https://profiles.ucsf.edu/nadav.brandes
* GitHub: https://github.com/nadavbra/shared_utils
* Licence: MIT
"""
...
def compare_list_against_collection(input_list, collection):
    ...
    return [element for element in input_list if element in collection_set], [element for element in input_list if
                                                                              element not in collection_set]
...
def add_parser_task_arguments(parser):
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='''If you want to distribute this process across multiple computation resources (e.g. on a cluster) you 
        can specify the total number of tasks (--total-tasks) to split it into, and the index of the current task to 
        run (--task-index).'''
    )
    parser.add_argument(
        '--total-tasks',
    ...
    parser.add_argument(
        '--task-index-env-variable',
        dest='task_index_env_variable',
        metavar='<e.g. SLURM_ARRAY_TASK_ID>',
        type=str,
        default=None,
        help='''Instead of specifying a hardcoded --task-index, you can specify an environment variable to take it 
        from (e.g. SLURM_ARRAY_TASK_ID if you use SLURM to distribute the jobs).'''
    )
    parser.add_argument(
        '--total-tasks-env-variable',
        dest='total_tasks_env_variable',
        metavar='<e.g. SLURM_ARRAY_TASK_COUNT>',
        type=str,
        default=None,
        help='''Instead of specifying a hardcoded --total-tasks, you can specify an environtment variable to take it 
        from (e.g. SLURM_ARRAY_TASK_COUNT if you use SLURM to distribute the jobs).'''
    )


def determine_parser_task_details(args, parser):
...
def find_linearly_independent_columns(matrix):
    """
    The calculation is fasciliated by the Gram Schmidt process, everytime taking the next column and removing its
    projections from all next columns, getting rid of columns which end up zero.
    """
    ...
    for i in range(n_cols):
        if not np.isclose(orthogonalized_matrix[:, i], 0).all():
            ...
            if i < n_cols - 1:
                ...
                orthogonalized_matrix[:, (i + 1):] -= np.dot(
                    orthogonalized_matrix[:, i],
                    orthogonalized_matrix[:, (i + 1):]
                ).reshape(1, -1) * orthogonalized_matrix[:, i].reshape(-1, 1)
...
def resolve_dummy_variable_trap(hot_encoding_df, validate_completeness=True, inplace=False, verbose=True):
    """
    When using one-hot-encoding in regression, there is a problem of encoding all possible variables if also using
    an intercept/const variable, because then the variables end up linearly dependent (a singular matrix is
    problematic with many implementations of regression).
    See for example: https://www.algosome.com/articles/dummy-variable-trap-regression.html
    To resolve this issue, this function will remove the most frequent column (to minimize the chance of any subset of
    the rows resulting a matrix which is not fully ranked).
    """
    ...
    if verbose:
        log('To avoid the "dummy variable trap", removing the %s column (%d matching records).' %
            (most_frequent_variable, hot_encoding_df[most_frequent_variable].sum())
            )
    ...
    if inplace:
        ...
        return hot_encoding_df[
            [column_name for column_name in hot_encoding_df.columns if column_name != most_frequent_variable]]
...
def are_close_dfs(df1, df2, rtol=1e-05, atol=1e-08):
    ...
    for column, dtype in df1.dtypes.iteritems():
        ...
        if np.issubdtype(dtype, np.float):
            cmp_series = np.isclose(df1[column], df2[column], rtol=rtol, atol=atol) \
                         | (pd.isnull(df1[column]) & pd.isnull(df2[column]))
        else:
...
def resolve_quasi_complete_separation_by_removing_binary_columns(X, y):
    """
    When performing logistic regression of y against X, the matrix X must be of full rank; otherwise
    (i.e. if the columns of X are linearly dependent), then statsmodel's Logit model gives a singular-matrix error.
    It also appears that quasi-complete separation causes troubles, namely if the columns of X are linearly dependent
    conditioned on y. In other words, assuming that y is binary, we need that X[y, :] would still be of full rank
    (we assume that the vast majority of records have a negative y value, and only a small fraction have a positive
    value, so given that X is of full rank we need not worry about X[~y, :]). To resolve this problem, this function
    will remove binary columns of X until X[y, :] is of full rank. Whenever a column of X is removed, we also remove
    the corresponding records (rows of X and y) that have those values (so if a removed column represent some covariate,
    e.g. a certain batch, we also remove all the samples from this batch in order for not having any covariates not
    accounted for).

    :param X: The exogenous variables (rows are records, columns are variables). (pd.DataFrame)
    :param y: The endogenous variable (must have the same index as X). (pd.Series)
    """
    ...
    # We sort the binary columns by how common they are, so when we start removing them, we will give priority to the
    # more common ones (i.e. remove the least frequent first).
    ...
    while len(all_kept_binary_columns) > 0:
        ...
        for column_name in columns_to_remove:
            log('Removing the columns %s (%d occurances) to avoid quasi-complete separation.' % (
                column_name, X[column_name].sum()))
    ...
    if not row_mask.all():
        log('Overall removed %d columns occuring in %d records to avoid quasi-complete separation.' % (
            len(all_removed_binary_columns), \
            (~row_mask).sum()))
    ...
    for upper_limit in range(1, int(max_mlog_pval + 3)):
        ...
        if len(range_pvals) > 0:
            ...
            if len(range_pvals) <= max_density:
                ...
                # We want to choose the p-values uniformly in the space of their expected frequencies (i.e. sampling
...
def draw_manhattan_plot(gwas_results, significance_treshold=5e-08, max_results_to_plot=1e06,
    ...
    CHROM_TO_COLOR = {'1': '#0100fb', '2': '#ffff00', '3': '#00ff03', '4': '#bfbfbf', '5': '#acdae9', '6': '#a020f1',
                      ...
                      '13': '#ffbfcf', '14': '#4484b2', '15': '#b63063', '16': '#f8816f', '17': '#ed84f3',
                      '18': '#006401',
                      '19': '#020184', '20': '#ced000', '21': '#cd0001', '22': '#050098', 'X': '#505050',
                      'Y': '#ff8000'}
    ...
    for chrom in CHROMS:
        if chrom in max_pos_per_chrom.index:
            chrom_accumulating_positions.append(
                (chrom, accumulating_pos + 1, accumulating_pos + max_pos_per_chrom[chrom]))
    ...
    chrom_accumulating_positions = pd.DataFrame(chrom_accumulating_positions, columns=['chromosome', \
                                                                                       'accumulating_start_position',
                                                                                       'accumulating_end_position']).set_index(
        'chromosome', drop=True)
...
# Slurm ---------------------------------------------------
def get_slurm_job_array_ids(parse_total_tasks_by_max_variable=True, log_ids=True, verbose=True,
                            task_index_remapping_json_file_path=None):
...
def liftover_loci_in_df(df, chrom_column='chromosome', pos_column='position', source_ref_genome='hg38', \
    ...
    return pd.concat(
        [new_chroms.rename(chrom_column) if
         column == chrom_column
         else
         (new_positions.rename(pos_column)
          if
          column == pos_column
          else
...
class Profiler(object):
    ...
    def format(self, delimiter='\n'):
        all_profiles = list(self.profiles.items()) + [
            ('Total', Profiler.Profile(total_invokes=1, total_time=datetime.now() - self.creation_time))]
