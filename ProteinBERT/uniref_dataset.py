...
class UnirefToSqliteParser:
    ...
    def _process_entry(self, i, event, entry):
        ...
        try:
            taxonomy_element, = db_ref.xpath(r'uniprot:property[@type="NCBI taxonomy"]',
                                             namespaces=UnirefToSqliteParser._NAMESPACES)
            tax_id = int(taxonomy_element.attrib['value'])
        except:
            tax_id = np.nan

        extracted_go_annotations = {category: UnirefToSqliteParser._extract_go_category(entry, category) for category in
                                    UnirefToSqliteParser._GO_ANNOTATION_CATEGORIES}
    ...
    def _save_current_chunk(self):
        chunk_records_df = pd.DataFrame(self._chunk_records, columns=['tax_id', 'uniprot_name', 'go_annotations'],
                                        index=self._chunk_indices)
        ...
        chunk_records_df['complete_go_annotation_indices'] = chunk_records_df['flat_go_annotations'].apply(
            self._get_complete_go_annotation_indices)
        ...
        chunk_records_df['complete_go_annotation_indices'] = chunk_records_df['complete_go_annotation_indices'].apply(
            json.dumps)
    ...
    def _extract_go_category(entry, category):
        return list(
            {property_element.attrib['value'] for property_element in entry.xpath(
                r'uniprot:property[@type="%s"]' %
                category,
                namespaces=UnirefToSqliteParser._NAMESPACES
            )}
        )
...
def parse_go_annotations_meta(meta_file_path):
    ALL_FIELDS = ['id', 'name', 'namespace', 'def', 'is_a', 'synonym', 'alt_id', 'subset', 'is_obsolete', 'xref',
                  'relationship', 'intersection_of', 'disjoint_from', 'consider', 'comment', 'replaced_by',
                  'created_by', 'creation_date', 'property_value']
...
def create_h5_dataset(protein_annotations_sqlite_db_file_path,
                      fasta_file_path,
                      go_annotations_meta_csv_file_path,
                      output_h5_file_path,
                      shuffle=True,
                      min_records_to_keep_annotation=100,
                      records_limit=None,
    ...
    common_annotation_ids = np.array(
        sorted(annotation_counts[annotation_counts >= min_records_to_keep_annotation].index)
    )
    original_annotation_index_to_common_annotation_index = {go_annotations_meta.loc[annotation_id, 'index']: i for
                                                            i, annotation_id in enumerate(common_annotation_ids)}

    if verbose:
        log('Will encode the %d most common annotations.' % len(common_annotation_ids))

    n_seqs = sum(1 for _ in
                 load_seqs_and_annotations(
                     protein_annotations_sqlite_db_file_path,
                     fasta_file_path,
                     shuffle=False,
    ...
    with h5py.File(output_h5_file_path, 'w') as h5f:
        ...
        h5f.create_dataset('included_annotations',
                           data=[annotation.encode('ascii') for annotation in common_annotation_ids],
        ...
        annotation_masks = h5f.create_dataset('annotation_masks', shape=(n_seqs, len(common_annotation_ids)),
                                              dtype=bool)

        start_index = 0

        for seqs_and_annotations_chunk in to_chunks(
                load_seqs_and_annotations(
                    protein_annotations_sqlite_db_file_path,
                    fasta_file_path,
                    shuffle=shuffle,
            ...
            annotation_masks[start_index:end_index, :] = _encode_annotations_as_a_binary_matrix(
                annotation_indices_chunk, original_annotation_index_to_common_annotation_index)
...
def load_seqs_and_annotations(protein_annotations_sqlite_db_file_path,
                              fasta_file_path,
                              shuffle=True,
                              records_limit=None,
                              verbose=True,
    ...
    raw_proteins_and_annotations = pd.read_sql_query(
        'SELECT uniprot_name, complete_go_annotation_indices FROM protein_annotations' + (
            '' if records_limit is None else
            (' LIMIT %d' % records_limit)), conn)

    if verbose:
        log('Loaded %d proteins and their GO annotations (%d columns: %s)' % (
                raw_proteins_and_annotations.shape + (', '.join(raw_proteins_and_annotations.columns),)))
